...
export const createStockLocation = createWorkflow(
  "create-stock-location",
  () => {
...
<CodeTabs group="resource-types">
  <CodeTab label="API Route" value="api-route">
    ...
    import { createStockLocation } from "../../workflows/create-stock-location"

    export async function GET(
      req: MedusaRequest,
      res: MedusaResponse
    ) {
      const { result } = await createStockLocation(req.scope)
        .run()
  ...
  <CodeTab label="Subscriber" value="subscriber">
    ...
    export default async function handleUserCreated({
      ...
      const { result } = await createStockLocation(container)
        .run()
