...
- `variant_id`: The ID of the product variant in the wishlist.
- `wishlist`: A relation to the `Wishlist` model, representing the wishlist the item belongs to.

You also define a unique index on the `id` and `variant_id` columns to ensure a product variant is added to the wishlist only once.
...
1. The first data model part of the link, which is the Wishlist Module's `wishlist` data model. A module has a special `linkable` property that contain link configurations for its data models.
...
You define a link between the `WishlistItem` data model and the `ProductVariant` data model. Instead of passing the `WishlistItem` data model's link configuration as a parameter similar to previous links, you pass an object having the following properties:

- `linkable`: The `WishlistItem` data model's link configuration. This is the link configuration you previously passed as a parameter.
- `isList`: A boolean indicating whether the link is a list. By enabling this property, you indicate that a product variant can be added as a wishlist item to multiple wishlists.

### Run Migrations to Sync Links

Links are created in the database as tables that store the associations between data models. To reflect these links on the database of the Medusa application, run the following command:
...
This will run any migration changes and sync any new links to the database.

In the next steps, you'll see how these links allow you to manage the associations between wishlists and customers, sales channels, and products.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        ...
        description: "Create a link between the wishlist and its customer and sales channel.",
        depth: 1,
...
The `createRemoteLinkStep` is from Medusa's workflows package. So, you'll only implement the first two steps.
...
export const validateCustomerWishlistHighlights = [
  ["13", "graph", "Retrieve the customer and its wishlist from the database"],
  ["21", "", "Throw an error if the customer doesn't exist"],
  ["28", "", "Throw an error if the customer already has a wishlist"],
]
...
In the step function, you use [Query](!docs!/learn/fundamentals/module-links/query) to retrieve the customer and its wishlist from the database. You throw an error if the customer doesn't exist, or if the customer already has a wishlist.
...
export const createWishlistWorkflowHighlights = [
  ["17", "validateCustomerCreateWishlistStep", "Validate that the customer doesn't already have a wishlist."],
  ["21", "createWishlistStep", "Create the wishlist."],
  ["23", "transform", "Create an array of links to create between the wishlist and the customer and sales channel."],
  ["47", "createRemoteLinkStep", "Create the links."],
  ["50", "wishlist", "Return the wishlist."]
]
...
- Use [transform](!docs!/learn/fundamentals/workflows/variable-manipulation) from the Workflows SDK to create an array of links to create between the wishlist and the customer and sales channel.
- Use the [createRemoteLinkStep](/references/helper-steps/createRemoteLinkStep) to create the links.
...
Learn more about creating links between records in [this documentation](!docs!/learn/fundamentals/module-links/link).
...
export async function GET(
  ...
  const { data } = await query.graph({
    ...
    fields: [
      "wishlist.*", 
      "wishlist.items.*", 
      "wishlist.items.product_variant.*",
    ],
    filters: {
      customer_id: req.auth_context.actor_id,
    },
...
In this route handler function, you use [Query](!docs!/learn/fundamentals/module-links/query) to retrieve the wishlist of the authenticated customer. Notice that you pass the link definition between a wishlist and a customer as an entry point to Query. This allows you to filter the wishlist by the customer ID.
  ...
  workflow={{
    ...
    steps: [
      ...
      },
      {
        type: "step",
        name: "createRemoteLinkStep",
        description: "Create a link between the wishlist item and the product variant.",
        depth: 1,
        link: "/references/helper-steps/createRemoteLinkStep"
      },
      {
...
The `useQueryGraphStep` and `createRemoteLinkStep` are from Medusa's workflows package. So, you'll only implement the other steps.
...
export const createWishlistItemWorkflowHighlights = [
  ["19", "useQueryGraphStep", "Retrieve the wishlist linked to the customer."],
  ["30", "validateWishlistSalesChannelStep", "Validate that the wishlist belongs to the specified sales channel."],
  ["36", "validateVariantWishlistStep", "Validate that the specified variant is not already in the wishlist."],
  ["42", "createWishlistItemStep", "Create the wishlist item."],
  ["47", "createRemoteLinkStep", "Create a link between the wishlist item and the product variant."],
  ["57", "useQueryGraphStep", "Retrieve the wishlist again with the new item added."],
  ["66", "wishlist", "Return the wishlist with the new item."]
]

```ts title="src/workflows/create-wishlist-item.ts" collapsibleLines="1-9" expandButtonLabel="Show Imports" highlights={createWishlistItemWorkflowHighlights}
...
- Use the [createRemoteLinkStep](/references/helper-steps/createRemoteLinkStep) to create a link between the wishlist item and the product variant.
...
export const deleteWishlistItemWorkflowHighlights = [
  ...
  ["26", "validateItemInWishlistStep", "Validate that the item is in the customer's wishlist."],
  ["31", "deleteWishlistItemStep", "Delete the wishlist item."],
  ["34", "useQueryGraphStep", "Retrieve the wishlist again with the item removed."],
  ["43", "wishlist", "Return the wishlist without the removed item."]
]
