...
export const metadata = {
  title: `Implement Localization with Contentful`,
  openGraph: {
    images: [
      {
        url: ogImage,
...
When you install a Medusa application, you get a fully-fledged commerce platform with a framework for customization. Medusa's architecture supports integrating third-party services to provide additional features, such as data localization.

[Contentful](https://www.contentful.com/) is a headless content management system (CMS) that allows you to manage and deliver content across multiple channels. By integrating Contentful with Medusa, you can manage your store's content, such as product titles and descriptions, in multiple languages. Then, you can show the product details in the storefront based on the user's language preference.

By following this tutorial, you'll integrate Contentful with Medusa to add localization capabilities to your store. You can follow this guide whether you're new to Medusa or an advanced Medusa developer.
...
- Trigger syncing products and related data to Contentful either when a product is created or when the admin user triggers syncing the products.
- Customize the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) to fetch localized data from Contentful.

<Note title="Good to Know">

- While you can localize the content of any model in the Medusa application, this tutorial focuses only on products. You can follow the same steps to add localization for other models, such as product collections or promotions.
- This tutorial doesn't cover how to localize the static content of the Next.js Starter Storefront, such as the homepage's content. You can implement that separately by integrating the Next.js Starter Storefront with Contentful, as well.
...
In this step, you'll create a custom module that provides the necessary functionalities to integrate Contentful with Medusa.
...
A module can specify a task to run on the Medusa application's startup using [loaders](!docs!/learn/fundamentals/modules/loaders). A loader is an asynchronous function exported by a module that Medusa runs when it starts. The loader can be used to perform one-time tasks such as connecting to a database, creating content types, or initializing data.

<Note>

Refer to the [Loaders](!docs!/learn/fundamentals/modules/loaders) documentation to learn more.
...
In the loader, you want to create content types in Contentful if they don't exist.

<Note>

As mentioned in the introduction, this tutorial focuses on localizing products only. So, if you want to localize other data models, make sure to create the necessary content types in the loader as well.
...
- `handle`: The product's handle, which is a non-localized field since this field is used to create a human-readable URL that you can use in the storefront.
- `medusaId`: The product's ID in Medusa, which is a non-localized field. This will allow you later to identify the product in Medusa.
...
- `medusaId`: The product variant's ID in Medusa, which is a non-localized field. This will allow you later to identify the product variant in Medusa.
...
- `medusaId`: The product option's ID in Medusa, which is a non-localized field. This will allow you later to identify the product option in Medusa.
...
- `medusaId`: The product option value's ID in Medusa, which is a non-localized field. This will allow you later to identify the product option value in Medusa.

You've now created all the necessary content types to localize products.

### Register Clients in Container

The last step in the loader is to register the Contentful management and delivery clients in the module's container. This will allow you to resolve and use them in the module's service next.

Replace the last `TODO` in the loader with the following:
...
To register resources in the container, you use the `register` method, which accepts an object of key-value pairs. The keys are the names of the resources in the container, and the values are the resources themselves.

So, you can now resolve the management and delivery clients in the module's resources using the keys `contentfulManagementClient` and `contentfulDeliveryClient`, respectively.

### Create Service

You define a module's functionalities in a service. A service is a TypeScript or JavaScript class that the module exports. In the service's methods, you can connect to the database, which is useful if your module defines tables in the database, or perform actions with a third-party service.
...
You export a class tha will be the Contentful Module's main service. In the class, you define properties for the Contentful clients and options passed to the module.
...
So, create the file `src/modules/contentful/index.ts` with the following content:
  ...
  modules: [
    {
      ...
      options: {
        management_access_token: process.env.CONTENTFUL_ACCESS_TOKEN,
...
- Set the fallback locale to the default locale. This ensure that values are retrieved in the default locale if values for the requested locale are not available.
...
- `CONTENTFUL_ACCESS_TOKEN`: The Contentful management API access token. To create it on the Contentful dashboard:
...
![The cog icon is at the top right next to the user avatar. If you click on it, a dropdown will show where you can click on "CMA tokens".](https://res.cloudinary.com/dza7lstvk/image/upload/v1744714244/Medusa%20Resources/Screenshot_2025-04-15_at_1.48.35_PM_ct2tfk.png)
    ...
    - The generated token is generated and shown in the pop-up. Make sure to copy it and use it in the `.env` file, as you can't access it again.
...
In addition, you can see on the Contentful dashboard that the content types you created in the loader. To view them, go to the Content Model page.
...
In this step, you'll implement the feature to create products in Contentful. You'll later use that feature when:
...
To create a product in Contenful, you'll need three methods: One to create the product's variants, another to create the product's options and values, and a third to create the product.
...
1. The product's variants, which is an array of objects.
...
In the method, you iterate over the product's options and create entries for each of its values. Then, you create an entry for the option. You set the fields based on the option and value content types you created earlier.
...
In the method, you first check if the product already exists in Contentful. If it does, you return the existing product entry. Otherwise, you create a new product entry. You set the fields based on the product content type you created earlier.
...
To implement the logic that's triggered when a product is created in Meduas, or when the admin user triggers a sync manually, you need to create a workflow.
...
In this section, you'll create a workflow that creates a product in Contentful when a product is created in Medusa.
...
3. A compensation function that undoes the actions performed in the step if an error occurs in the workflow's execution. This mechanism ensures data consistency in your application, especially as you integrate external systems.

<Note>

The Medusa container is different from the module's container. Since modules are isolated, they have a container with their own resources. Refer to the [Module Isolation](!docs!/learn/fundamentals/modules/isolation) documentation for more information.
...
2. Pass the retrieved products to the `createProductsContentfulStep` step.
...
## Step 4: Handle Product Creation

Medusa has an event system that allows you to listen to events, such as `product.created`, and perform an asynchronous action when the event is emitted.
...
- `event`: An object that holds the event's data payload.
...
You can also see the product in the Contentful dashboard.

---

## Step 5: Trigger Product Sync Manually

The other way you'll allow syncing the product is when the admin user triggers a sync manually. This is useful when you already have products in Medusa and you want to sync them to Contentful.

To allow admin users to trigger a sync manually, you need:

1. A subscriber that listens to a custom event.
2. An API route that emits the event when a request is sent to it.
...
In the route handler, you resolve the [Event Module](../../../architectural-modules/event/page.mdx)'s service from the Medusa container to emit the `products.sync` event.

### Create UI Route to Trigger Sync

Finally, you'll add a new page to the Medusa Admin dashboard that displays a button to trigger the sync.
...
You'll now create the UI route that displays a button to trigger the reindex. UI routes are created in a `page.tsx` file under a sub-directory of `src/admin/routes` directory. The file's path relative to `src/admin/routes` determines its path in the dashboard.
...
The first method retrieves all locales from Contentful. Add the following method to the service at `src/modules/contentful/service.ts`:
...
The second method returns the code of the default locale. Add the following method to the service:
...
Then, you format the locales to return their name, code, and whether they are the default locale.

Finally, you return the formatted locales in the JSON response.

### Customize Storefront to Show Locales

In the first step, you installed the [Next.js Starter Storefront](../../../nextjs-starter/page.mdx) along with the Medusa application. This storefront provides standard ecommerce features, such as a product catalog, a cart, and a checkout.

In this section, you'll customize the storefront to show the locales to customers and allow them to select from them. The selected locale will be stored in the browser's cookies, allowing you to use them later when retrieving a product's localized data.
...
```ts title="src/lib/data/cookies.ts" badgeLabel="Storefront" badgeColor="blue"
...
You create a `LocaleSelect` component with the following variables:
...
Finally, you'll show the locale select in the side menu after the country selector.
...
The next feature you'll implement is retrieving and showing product details for a selected locale.

You'll implement this feature by:

1. Linking Medusa's product to Contentful's product.
2. Add the method to retrieve product details for a selected locale from Contentful.
3. Add a new route to retrieve the product details for a selected locale.
4. Customize the storefront to show the product details for the selected locale.
...
To create the API route, create the file `src/api/store/products/[id]/[locale]/route.ts` with the following content:
...
Since you export a `GET` route handler function, you expose a `GET` route at `/store/products/[id]/[locale]`.
...
- `context`: An additional context to be passed to the methods retrieving the data.

By specifying `contentful_product.*` in the `fields` property, Medusa will retrieve the product's entry in Contentful using the `list` method you just added. Medusa passes the filters and context to the method, and attaches the returned data to the Medusa product if its `product_id` matches the product's ID.
...
A rich-text field that is returned by Contentful is an object that requires special rendering. So, Contentful provides a package to render rich-text fields.
...
In `src/lib/data/locale.ts`, add the following:
...
Then, in the `ProductPreview` component, add the following before the `return` statement:
...
Finally, to show the localized product title, find in the `return` statement the following line:
...
The product's details page is defined in `src/app/[countryCode]/(main)/products/[handle]/page.tsx`. So, add the following import in the file:

```tsx title="src/app/[countryCode]/(main)/products/[handle]/page.tsx" badgeLabel="Storefront" badgeColor="blue"
import { getProductLocaleDetails } from "@lib/data/locale"
```

Then, in the `ProductPage` component, add the following before the `return` statement:
...
Finally, in the `return` statement, pass the following prop to `ProductTemplate`:
...
In this step, you'll setup a webhook listener that updates the product's data when a Contentful entry is published.

### Prerequisites: Public Endpoint

Webhooks can only trigger deployed listeners. So, you must either [deploy your Medusa application](../../../deployment/page.mdx), or you can use tools like [ngrok](https://ngrok.com/) to expose your local application to the web.

### Setup Webhooks in Contentful

Before setting up the webhook listener, you need to setup a webhook in Contentful. To do that, on the Contentful dashboard:
...
1. Add support for other data types, such as product categories.
2. Listen to other product events and update the Contentful entries accordingly. Refer to the [Events Reference](../../../events-reference/page.mdx) for details on all events emitted in Medusa.
3. Add localization for the entire Next.js Starter Storefront. You can either create content types in Contentful for different sections in the storefront, then use them to retrieve the localized content, or use the approaches recommended in the [Next.js documentation](https://nextjs.org/docs/app/building-your-application/routing/internationalization).
4. Listen to webhooks from Contentful to update Medusa's data accordingly. This depends on your use case and how you choose to manage your data. Refer to [Contentful's Webhooks](https://www.contentful.com/developers/docs/webhooks/) documentation to learn more.
