...
Medusa facilitates error tracking and performance monitoring by integrating instrumentation tools like [Sentry](https://sentry.io/). Sentry provides real-time tracking of errors and performance issues in your application, allowing you to identify and fix them quickly.
...
- Set up instrumentation with Sentry. You'll be tracing HTTP requests, workflows, Query usages, and database operations.
- Test out monitoring and error tracking.
...
You can now test out your Sentry integration with your Medusa application.
...
In this section, you'll test it out by creating a custom API route that throws an error, which you'll then view in Sentry.
...
In the route handler, you throw a `MedusaError` with a custom message. This error will be returned as a response to the request and will also be available in the Sentry tracing.

To test out the API route, make sure your Medusa application is running, then go to `http://localhost:9000/test-sentry` in your browser or using `curl`:
...
If you click on the Trace ID, you'll see the details of the trace. Click on the first span in the trace and scroll down to the "Breadcrumbs" section to see the error details.
...
You've now integrated Sentry with your Medusa application. You can keep using Sentry moving forward to monitor your application's performance and track errors.
