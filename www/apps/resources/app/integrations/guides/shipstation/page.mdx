...
ShipStation is a shipping toolbox that connects all your shipping providers within one platform. By integrating it with Medusa, you can allow customers to choose from different providers like DHL and FedEx, calculate the shipping rate for each shipping method, and process order fulfillment with the chosen provider.
...
To integrate ShipStation using their API, you must enable its Add-On. To do that:

1. Go to Add-Ons from the navigation bar.

![Click on Add-Ons in the navigation bar](https://res.cloudinary.com/dza7lstvk/image/upload/v1734337981/Medusa%20Resources/Screenshot_2024-12-16_at_10.32.40_AM_u2m1bi.png)
...
o integrate third-party services into Medusa, you create a custom module. A module is a re-usable package with functionalities related to a single feature or domain. Medusa integrates the module into your application without implications or side effects on your setup.
...
A Fulfillment Module Provider service must extend the `AbstractFulfillmentProviderService` class. You'll implement the abstract methods of this class in the upcoming steps.

The service must also have an `identifier` static property, which is a unique identifier for the provider. You set the identifier to `shipstation`.

A module can receive options when it's later added to Medusa's configurations. These options allow you to safely store secret values outside of your code.

The ShipStation module requires an `api_key` option, indicating your ShipStation's API key. It also accepts a `sandbox` option which, when enabled, sends requests to ShipStation's mock server. You receive the options as a second parameter of the constructor.
...
You also add a private `sendRequest` method that sends a request to a specified URL with request data. In the method, you determine the base URL of the request based on whether the `sandbox` option is enabled. You then send a request using the Fetch API, passing the API key from the options in the request header.
...
ShipStation requires that a shipment must be associated with a carrier and one of its services. So, in this method, you'll retrieve the list of carriers from ShipStation and return them as fulfillment options.
...
You define a `Carrier` type that holds a carrier's details, and a `CarriersResponse` type, which is the response returned by ShipStation when retrieving carriers.
...
export const serviceHighlights3 = [
  ...
  ["12", "filter", "Filter out carriers that are disabled by the billing plan."],
  ["14", "services", "Loop over the services that the carrier provides."],
  ["15", "push", "Add an option for the carrier and service combination."]
]
...
In the `getFulfillmentOptions` method, you retrieve the carriers from ShipStation. You then filter out the carriers disabled by the billing plan, and loop over each carrier and its services.

For each carrier-service pair, you add a new item to the fulfillment options that are returned by this method. Each fulfillment option is an object that must have an `id` property, which you set to the carrier's ID. You can pass other data, such as the `carrier_service_code`, and Medusa will store all the fulfillment option data in the shipping option created later.
...
So, in the `validateFulfillmentData` method of your provider, you'll use that endpoint to create a shipment with rates for carrier and service code associated with the selected shipping option, then store the ID of that shipment in the created shipping method's data.
...
Next, add to `ShipStationClient` a method to send a request to [ShipStation's get shipping rates endpoint](https://docs.shipstation.com/openapi/rates/calculate_rates):
...
The `getShippingRates` accepts the request body of the [ShipStation's get shipping rates endpoint](https://docs.shipstation.com/openapi/rates/calculate_rates). It uses the `sendRequest` method to send a request to ShipStation, and returns the endpoint's response.
...
In the method, you prepare the data to get the shipping rates and create the shipment in ShipStation, including the carrier ID, the address to ship from and to, and more.
...
export const serviceHighlights5 = [
  ...
  ["25", "is_calculated_price_tax_inclusive", "Return whether the price includes taxes."]
]
...
After the customer places the order, the admin user can manage it including its fulfillments. When the admin user creates a fulfillment for the order, Medusa uses the `createFulfillment` method of the associated provider to handle any processing in the third-party provider.
...
export const serviceHighlights6 = [
  ...
  ["17", "label", "Purchase a label in ShipStation."],
  ["19", "shipment_data", "Pass the shipment's details."],
  ["20", "items", "Set the items to be fulfilled."],
  ["29", "outbound_label_id", "Set the ID of the original shipment."],
  ["33", "label_id", "Store the label's ID in the fulfillment's `data` property."]
]
...
- `SHIPSTATION_SANDBOX`'s value is `true`, indicating that requests shouldn't be sent to the ShipStation's production server. You can change this to `false` when the integration is production-ready at the end of this guide.
