...
export const serviceHighlights8 = [
  ...
  ["23", "optionData", "Retrieve the carrier ID and service code from the shipping option's `data`."],
  ["34", "ship_from", "Prepare the address to ship items from."],
  ["52", "ship_to", "Prepare the address to ship items to."]
]
...
class ShipStationProviderService extends AbstractFulfillmentProviderService {
  ...
  private async createShipment(
    ...
    const ship_from: ShipStationAddress = {
      ...
      phone: context.from_location?.address?.phone || "",
      address_line1: context.from_location?.address?.address_1 || "",
      city_locality: context.from_location?.address?.city || "",
      state_province: context.from_location?.address?.province || "",
      postal_code: context.from_location?.address?.postal_code || "",
      country_code: context.from_location?.address?.country_code || "",
    ...
    const ship_to: ShipStationAddress = {
      ...
      address_line1: context.shipping_address.address_1 || "",
      city_locality: context.shipping_address.city || "",
...
The `createShipment` method accepts the following parameters:
...
These parameters are passed to the `calculatePrice` method and others as well, so you can reuse this `createShipment` method in the other methods.

In the `createShipment` method, so far you only prepare the data to be sent to ShipStation. ShipStation requires the address to ship the items from, which is the address of the stock location associated with the chosen shipping option, and the address to ship items to, which is the customer's address.
...
class ShipStationProviderService extends AbstractFulfillmentProviderService {
  ...
  async calculatePrice(
    ...
    if (!shipment_id) {
      const shipment = await this.createShipment(optionData, data, context)
    ...
    const calculatedPrice = !rate ? 0 
      : rate.shipping_amount.amount + rate.insurance_amount.amount + 
...
In this method, you'll purchase a label for the shipment that was previously created for the associated shipping method. You'll also store the ID of the label and shipment within the created fulfillment's `data` property.
...
Then, add the following method to the `ShipStationClient`:
...
You add the `purchaseLabelForShipment` method to purchase a label in ShipStation for a shipment by its ID.
...
export const serviceHighlights6 = [
  ...
  ["13", "label", "Purchase a label in ShipStation."],
  ["16", "data", "Return the data to be stored in the fulfillment's `data` property."]
]
...
class ShipStationProviderService extends AbstractFulfillmentProviderService {
  ...
  async createFulfillment(
    ...
    const label = await this.client.purchaseLabelForShipment(shipment_id)
...
In the method, you retrieve the shipment ID from the associated shipping method's `data` property, then use the client's `purchaseLabelForShipment` method to purchase the label.
