...
<SourceCodeLink link="https://github.com/medusajs/medusa/blob/911eefadbd8280509f7bf89c1fa2e1b083c1d219/packages/core/core-flows/src/invite/workflows/create-invites.ts#L15" />
...
export async function POST(
  ...
  const { result } = await createInvitesWorkflow(req.scope)
    .run({
      input: {
        "invites": [{
          "email": "Raphaelle_Schaden68@yahoo.com"
        }]
      }
    })
