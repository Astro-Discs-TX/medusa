...
This isolation between environment databases allows you to safely test changes in other environments without affecting the production database.
...
In some cases, you may need to access the database directly, such as to export or import data. However, since Cloud is a managed service, you can't directly access the database server.

This section provides alternative ways to interact with your database.

### Option 1: Database Dump

Cloud allows you to export and import database dumps for any environment. This is useful for backing up data, seeding the database with initial data, or debugging issues locally.
...
- **Export**: Download the current database as a dump file. You'll then receive a notification once the export is ready for download. You can restore the database dump locally using `pg_restore`. For example:

```bash
pg_restore --no-acl --no-owner --no-privileges --clean --if-exists -d 'postgres://postgres:secret@localhost:5432/mydatabase' <filename>
```
...
To interact with the database with more flexibility, you can:
...
This approach allows you to perform any database operations you need, such as retrieving or updating data, without directly accessing the database server.
...
Cloud also allows you to configure which environment's database to replicate from. For example, you can replicate the Staging environment's database instead of Production.
