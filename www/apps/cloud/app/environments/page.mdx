...
- Environments are isolated spaces for different stages of development, such as production, staging, and development.
- An environment can be created for a branch in the GitHub repository.
- Each environment has its own set of configurations, secrets, and resources.
- Environments are useful for testing changes in isolation before merging them into the main branch.
- By default, a project has a production environment.
- How to create a new environment.
- Talk about how to view an environment's dashboard and details
...
- How to manage environments
  ...
  - Link to viewing environment logs.
- Preview Environments
  - Preview environments are automatically created for each pull request. (Show screenshot)
  - They allow you to test changes in a live environment before merging.
  - How to manage general settings for preview environments.
  - How to view a preview environment's dashboard and details.
  - How to manage settings of a specific preview environment.
- How to delete an environment.

*/}

In this guide, you'll learn about environments, how to create and manage them, how they help you in development, and more.

## What is an Environment?

An environment is an isolated space for different stages of a project, such as production and staging. A project has at least a Production environment by default, and you can create more environments based on your development needs.

You can create an environment for a specific branch in your GitHub repository, and each environment has its own configurations, secrets, and resources.

Environments are useful to test changes in production-like conditions before actually merging them into the main branch, which is the production environment. You can test out new features, bug fixes, and other changes without affecting the live application.

### Environment Types

There are two types of environments:

- Long-lived environments: These are environments that you create for specific branches in your GitHub repository, such as Staging or Production.
- Short-lived or Preview environments: These are automatically created for each pull request (PR) towards a branch that has a long-lived environment in Cloud. They're created as a preview for the PR, and are destroyed once the PR is merged or closed.
...
2. Click on the project you want to view its environments.
3. In the project's dashboard, you'll find a card for every environment in your project.
...
As mentioned earlier, an environment is associated with a specific branch in your GitHub repository. So, before creating an environment, make sure you have the necessary branch for it in your repository.

For example, if you're creating a Staging environment, create a `staging` branch in your repository:
...
Then, to create the environment in the Cloud dashboard:
...
2. In the side window that opens:
    ...
    - Set the custom subdomain for the environment. All environments are subdomains of `medusajs.app`. So, if you set the subdomain to `staging-my-project`, the environment will be accessible at `staging-my-project.medusajs.app`.
...
After you create an environment, it will appear in the project's dashboard. The environment will have its own resources, including its own database, separate from the production environment.

Then, make a commit to the environment's branch. Cloud will deploy the changes to that environment.

<Note>

Making a commit for the first time is required to trigger the deployment of the environment. After that, any changes you make to the branch will automatically trigger a deployment to the environment.
...
### Access Medusa Admin the First Time

To access the Medusa Admin for a custom environment, you need to set the email and password for the admin user.
...
5. Click on the "Overview" tab to go back to the environment's dashboard.
6. Click on the "Redeploy" button to redeploy the environment with the new environment variables.

Wait for the deployment to finish, which you can check its status in the "Deployments" section.

Once the deployment is finished, you can access the Medusa Admin by going to the environment's URL, then logging in.

---

## Access Environment Server through SSH

Cloud doesn't support SSH access to the server instance of an environment. However, you can still access the server's [runtime and build logs](#) to debug issues or monitor the application's performance.
...
This will open the environment's dashboard, where you can see its details, logs, and settings. You can also see the environment's name at the top left of the Cloud dashboard.
...
The environment's name at the top left of the dashboard is also an environment switcher.

To switch to a different environment:

1. Click on the environment's name at the top left of the Cloud dashboard, next to the project name.
2. Choose the environment you want to switch to from the dropdown.

![Environment switcher dropdown at the top left of the dashboard](https://res.cloudinary.com/dza7lstvk/image/upload/v1749815237/Cloud/CleanShot_2025-06-13_at_14.46.54_2x_j4848b.png)
...
You can trigger the deployment of an environment either by pushing a commit to the environment's branch, or by clicking the "Redeploy" button at the top right of the environment's dashboard.
...
In an environment's "Settings" tab, you can edit the environment's variable and secrets.
...
2. In the "Add new environment variables" section, there are different ways you can add environment variables:
    ...
    - **Paste `.env` format**:
      - If you have environment variables in a `.env` file format, you can paste them directly into the "Key" field. The environment variables will be pre-filled for you.
    - **Import `.env` file**:
      - If you have a `.env` file, click the "Import .env" button to upload the file. The environment variables will be automatically extracted and added to the list.
3. Once you're done, click the "Save" button to save the environment variables.

After adding the environment variables, you must [redeploy the environment](#trigger-environment-deployment) for the changes to take effect.
...
After editing the environment variable, you must [redeploy the environment](#trigger-environment-deployment) for the changes to take effect.
...
After deleting the environment variable, you must [redeploy the environment](#trigger-environment-deployment) for the changes to take effect.
...
You can export the environment variables of an environment as an `.env` file. This is useful for debugging issues that require you to have the same environment variables.
...
You can rename the file to `.env` if you want to use it directly in your local environment.
...
A preview environment is a short-lived environment that is automatically created for each pull request (PR) towards a branch that has a long-lived environment in Cloud, such as Staging or Production.

Preview environments facilitate testing a PR's changes in a live environment before merging them into the main branch. They allow you to see how the changes will affect the application in a production-like environment, without affecting the live application.
...
### Trigger Preview Environment Deployment

To trigger a new preview environment, create a PR towards a branch that has a long-lived environment in Cloud, such as Production.

Cloud will automatically create a preview environment for the PR, and deploy the changes to that environment.

You can also view the preview deployment and its status as a comment on your PR in GitHub.
...
#### Preview Environment's Resources

Each preview environment has its own resources, including its own database, separate from the production environment.

By default, the preview environment's database will be replicated from the Production environment's database. However, you can change this in the project's [Previews settings](#manage-previews-settings).

In addition, preview environments are put to sleep after ten minutes of inactivity. So, if you're testing scheduled jobs or events, it's normal that they may not work as expected in preview environments. They will work as expected in long-lived environments, such as Staging or Production.

### Access Deployed Preview Environment

Once the preview environment is finished deploying, you can access its Medusa Admin either through the comment in the PR, or by clicking the "Preview" button in the project's dashboard.
...
To open a preview environment's dashboard, click on the preview environment in the "Previews" card of the project's dashboard.

Then, you can manage the preview environment just like any other long-lived environment, as mentioned throughout this guide. You can edit its details, environment variables, deployment rules, and more.
