...
An environment is an isolated space for different stages of a [project](../projects/page.mdx) that has its own resources and configurations. For example, you can have Production and Staging environments.
...
An environment is associated with a specific branch in your GitHub repository. So, to create a Staging environment, you need a `staging` branch in your repository.
...
In the project's dashboard, you'll find a card for every environment in your project.
...
It's recommended to create long-lived environments for different stages of your project. For example, you can create a Staging environment to test new features and updates before deploying them to Production. 

That way, you can test changes in a production-like environment without affecting the live application.
...
After you create an environment, it will appear in the [project's dashboard](../projects/page.mdx) as a card.

### Trigger Environment Deployment

After creating the environment, you need to trigger its deployment by pushing a commit to the environment's branch. The first time you push a commit, Cloud will:

- Deploy the environment with the latest code from the associated branch.
...
After the environment is deployed, you need to set the email and password for the admin user before you access the Medusa Admin.
...
After adding environment variables, you need to redeploy the environment for the changes to take effect:
...
Wait for the deployment to finish. You can check its status in the "Deployments" section. Once it's deployed, you can [access the Medusa Admin](#access-medusa-admin).
...
You can export the environment variables of an environment as a `.env` file. This is useful for debugging issues that require you to have the same environment variables.
...
You can rename the file to `.env` if you want to use it in your local development.
...
Preview environments facilitate testing a PR's changes in a live environment before merging them into the main branch. They allow you to test changes in an environment that closely resembles the production environment.
...
### Create Preview Environment Deployment
...
By default, the preview environment's database will be replicated from the Production environment's database.
...
#### Limitations of Preview Environments

Preview environments are put to sleep after ten minutes of inactivity. So, if you're testing scheduled jobs or events, they may not work as expected in preview environments. They will work as expected in long-lived environments, such as Staging or Production.

### Delete Preview Environment

Preview environments are automatically deleted when the PR is merged or closed. However, you can also delete them manually if needed.
