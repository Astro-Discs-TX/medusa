...
A deployment is a live version of an environment that is publicly accessible. You can access it at the [environment's URL](#find-environments-url).
...
Deployments can also be redeployed as explained in the [Redeploy a Deployment](#redeploy-a-deployment) section.
...
2. You can go to the latest Production deployment by clicking the "Latest Deployment" link in the Production environment card.
...
These steps apply to all environments in your project, including custom environments (like Staging) and preview environments.
...
2. **Status**: The current status of the deployment. For example, "Live" or "Build Failed".
...
Once a deployment is live, you can access it through its environment's URL.
...
3. You can also click on the environment's name, where you'll find in its dashboard the URL under the environment's name.
...
You can then login using the email and password set either [during project creation](../projects/page.mdx#create-a-project) or [in the environment's variables](../environments/page.mdx#create-medusa-admin-user-in-environment).
...
### Access Deployment Server through SSH
...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        The build process was canceled due to a newer commit pushed to the branch.
      </Table.Cell>
    </Table.Row>
    <Table.Row>
      <Table.Cell>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        The deployment was previously deployed, but it's not currently the live version of the environment. This happens when a new deployment became the live version.
      </Table.Cell>
    </Table.Row>
  </Table.Body>
</Table>
...
You might need to redeploy a deployment if there are unexpected issues with the current live deployment. By redeploying a previous deployment, you revert or rollback the live version of its environment to the code in that deployment.
