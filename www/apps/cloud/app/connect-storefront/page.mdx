...
In this guide, you'll learn how to configure your Medusa application on Cloud and production storefront to work together.

## Cloud and Storefront Hosting Overview

Cloud only hosts Medusa applications. So, it doesn't support hosting your storefronts. Instead, you can use hosting providers like [Vercel](https://vercel.com/) to host your storefronts.
...
Once your storefront is hosted, you need to configure it and your Medusa application deployed on Cloud to work together. Otherwise, your storefront can't send requests to your Medusa application.
...
- `STORE_CORS`: The URLs of storefronts that can access your Medusa server. This is a comma-separated list of URLs that is typically used as the value of the [http.storeCors](!docs!/learn/configurations/medusa-config#httpstorecors) configuration
- `AUTH_CORS`: The URLs of the clients (storefront and admin) that can access the Medusa server. This is a comma-separated list of URLs that is typically used as the value of the [http.authCors](!docs!/learn/configurations/medusa-config#httpauthcors) configuration.
...
- `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY`: A publishable API key from your production Medusa application. You can [create or use an existing one from the Medusa Admin](!user-guide!/settings/developer/publishable-api-keys).

<Note>

These environment variables are used by the Next.js Starter Storefront to connect to your Medusa application. If you're using a different storefront, you may need to set different environment variables.
