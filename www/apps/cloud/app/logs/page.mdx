...
Logs are output messages from your Medusa application's build and runtime processes for an environment.

They include messages throughout your application's lifecycle, including messages during build, application startup, and runtime operations.
...
There are two log types in your environment:
...
This will open the deployment details page, where you can see the build logs.
...
You can search the build logs using the search bar at the top of the "Build logs" section. This is useful for finding specific messages or errors in the logs.
...
In addition, you can expand or collapse individual steps by clicking the arrow next to step titles. This gives you more control over which parts of the logs you want to see or hide.
...
To pause the build logs, click the "Pause" button at the bottom right of the page. This button is only shown when there are incoming logs. When you pause the logs, the button will change to "Resume", allowing you to resume the live log updates.
...
Runtime logs are useful for debugging:

- Application startup.
- HTTP requests and their errors.
- Messages or errors logged by scheduled jobs and subscribers.
- Other runtime issues that may occur while the application is running.

You can only view the runtime logs of an environment's live deployment.

To view the runtime logs for an environment:
...
For example, you can search for keywords like "error" or HTTP request paths to quickly locate issues in the runtime logs.
...
- **Timeline**: Select a specific time range to view logs from that period. For example, you can view the logs from the last hour, or select a custom time range.
...
You can apply filters from the "Filters" sidebar. You can select multiple filters at once to narrow down the logs further.
...
You can refresh the shown logs by clicking the <InlineIcon Icon={ArrowPath} alt="refresh" /> button at the top right of the Logs page. This will fetch the latest logs from the live deployment and update the view.
...
Copying the log message or JSON details is useful for sharing with your team or when contacting support for help debugging an issue.
