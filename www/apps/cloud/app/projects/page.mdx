...
So, you don't need to add any of these modules to your Medusa application before deploying it to Cloud, unless you want to add custom options to them.

#### S3 File Provider Module Options

If you want to add custom options to the S3 File Provider Module, make sure to include the `authentication_method: "s3-iam-role"` option.

For example:

```ts title="medusa-config.ts" highlights={[["12"]]}
