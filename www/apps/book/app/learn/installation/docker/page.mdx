...
If you try to run the Medusa server now with Docker, you'll encounter an SSL error as the server tries to connect to the PostgreSQL database.
...
## 10. Start the Medusa Server with Docker

All configurations are now ready. You can start the Medusa server using Docker by running the following command:
...
Once you see the following message, the Medusa server is ready:

```shell
Medusa server is running on http://localhost:9000
```
...
## Stop the Medusa Server Running in Docker

To stop the Medusa server running in Docker, run the following command:
...
You can check the logs of the Medusa server running in Docker using the following command:

```bash
docker compose logs -f medusa
```

This command shows the logs of the `medusa` service, allowing you to see any errors or messages from the Medusa server.
