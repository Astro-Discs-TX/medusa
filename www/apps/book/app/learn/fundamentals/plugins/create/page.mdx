...
- `src/provider`: Contains module providers, which require some additional setup as explained in [this section](#building-module-providers).
...
For module providers, refer to [this section](#building-module-providers).
...
### Building Module Providers

If you're building a module provider, such as a Notification Module Provider, you need to change how your plugin package exports files. That's because a module provider is registered as part of another module, and not automatically registered like other modules.

In your plugin project, add an `exports` field to your `package.json` file:
...
You export the following files and directories:
...
And you can register a provider in the Medusa application's `medusa-config.ts` like this:
