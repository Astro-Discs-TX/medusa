...
Plugins are available starting from [Medusa v2.2.1](https://github.com/medusajs/medusa/releases/tag/v2.2.1).
...
- `src/api`: Contains [API routes](../../api-routes/page.mdx). You can add store, admin, or any custom API routes.
...
Next, navigate to your Medusa application and run the following Medusa CLI command to install the plugin:
...
Make sure to replace `~/path/to/medusa-app` with the path to your Medusa application and `@myorg/plugin-name` with the name of your plugin as specified in `package.json`.
...
If your plugin has modules that expect options, you can pass the options through the plugin configuration in the `medusa-config.ts` file.
