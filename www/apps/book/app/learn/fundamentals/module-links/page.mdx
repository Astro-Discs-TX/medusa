...
### How Module Links are Stored

When you define a module link, the Medusa application creates a table in the database for that link. The table's name is a combination of the names of the two data models linked in the format `module1_table1_module2_table2`, where:
...
![Diagram illustration for links](https://res.cloudinary.com/dza7lstvk/image/upload/v1741696766/Medusa%20Book/custom-links_vezsx8.jpg)
...
## Renaming Participants in a Link

As mentioned in the [How Module Links are Stored](#how-module-links-are-stored) section, the name of a link's table consists of the names of the modules and the data models' table names.
...
To delete a link definition, remove the link file from the `src/links` directory. Then, run the `db:sync-links` or `db:migrate` command to delete the link table from the database:
