...
const post = await postRepository.create({
  title: "Introduction to Medusa",
  content: "Medusa is a composable commerce platform...",
...
const post = await postRepository.findOne({
  where: { id: "post_123456" }
})

// Update a post
await postRepository.update("post_123456", {
  status: "published",
  metadata: {
    featured: true
  }
...
// Delete a post
await postRepository.delete("post_123456")

// Find multiple posts with filtering
...
You can also create custom repository classes with specialized query methods:

```ts
import { DALUtils } from "@medusajs/framework/utils"
import { Post } from "./models/post"

export class PostRepository extends DALUtils.mikroOrmBaseRepositoryFactory(Post) {
...
Here's a more practical example demonstrating multiple operations in a single transaction:

```ts
await postRepository.transaction(async (transactionManager) => {
  // Create a new blog post
  ...
  // Update author statistics in the same transaction
  const authorStats = await transactionManager.findOne(AuthorStats, {
    where: { author_id: post.author_id }
  })
  
  authorStats.post_count = authorStats.post_count + 1
  
  // Save both changes atomically
  await transactionManager.persistAndFlush([post, authorStats])
...
Here are examples of more complex SQL queries for a blog module:

```ts
// Get popular posts with their categories
