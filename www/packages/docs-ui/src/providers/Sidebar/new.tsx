...
export type SidebarActionOptions = {
  sidebar_id: string
  parent?: {
    type: SidebarNew.InteractiveSidebarItemTypes
    path: string
    title: string
...
export type SidebarContextType = {
  ...
  isItemActive: (item: SidebarNew.InteractiveSidebarItem) => boolean
  addItems: (
  ...
  getSidebar: (
    sidebar_id: string
  ) => SidebarNew.Sidebar | SidebarNew.SidebarItemSidebar
} & SidebarStyleOptions
        ...
        ...state.slice(0, sidebarIndex),
        {
          ...
          items: sidebar.items.map((i) => {
            ...
            if (
              ...
              areSidebarItemsEqual({
                itemA: i,
                itemB: parent as SidebarNew.InteractiveSidebarItem,
              })
            ) {
              return {
                ...i,
...
export const SidebarNewProvider = ({
  children,
  isLoading,
  setIsLoading,
  ...
  const { activeItem, sidebarHistory, parentSidebar } = useMemo(() => {
    ...
    return {
      ...
      activeItem: (result.item as SidebarNew.SidebarItemLink) || null,
    }
    ...
    (item: SidebarNew.InteractiveSidebarItem): boolean => {
      ...
      return (
        item.children?.some((child) =>
          areSidebarItemsEqual({
            itemA: child,
            itemB: { type: "link", path: activePath, title: "" },
            compareTitles: false,
          })
        ) || false
      )
  ...
  const addItems = (
    newItems: SidebarNew.SidebarItem[],
    options?: SidebarActionOptions
  ) => {
    dispatch({
...
export const useSidebarNew = (): SidebarContextType => {
  const context = useContext(SidebarContext)

  if (!context) {
    throw new Error("useSidebarNew must be used inside a SidebarNewProvider")
  }
