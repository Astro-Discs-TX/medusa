...
export type ItemsToAdd = SidebarNew.SidebarItem & {
...
async function getAutogeneratedTagSidebarItems(
  tags: string,
  type: "link" | "ref",
  existingChildren?: SidebarNew.RawSidebarItem[]
): Promise<ItemsToAdd[]> {
  const items: ItemsToAdd[] = []
...
async function checkItem(
  item: SidebarNew.RawSidebarItem
): Promise<SidebarNew.RawSidebarItem> {
  if (!item.type) {
    throw new Error(
  ...
  item.children = sortSidebarItems({
    items: item.children as SidebarNew.RawSidebarItem[],
...
export async function generateSidebar(
  sidebars: SidebarNew.RawSidebar[],
  options?: GenerateSidebarOptions
): Promise<void> {
  const path = await import("path")
  const { writeFileSync } = await import("fs")

  const normalizedSidebars: SidebarNew.RawSidebar[] = []
